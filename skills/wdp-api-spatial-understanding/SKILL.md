...
> 🚨 **不要假设返回结构**
> WDP API 返回的 `result` 可能是对象、数组、或分类映射，必须先查阅 `official-*.md` 获取 API 的具体返回结构再编码。
>
> 🚨 **实体需二次查询**
> `GetAll()` / `GetByTypes()` 返回的 `entityObj` 只是引用，调用 `entityObj.Get()` 才能获取名称、ID 等完整属性。
...
**核心能力**：
- **获取默认/当前坐标系**：通过 `App.Scene.GetGlobal()` 获取当前生效的 `result.geoReference`。
- **自定义参考坐标系 (CAD对齐必用)**：通过 `new App.GeoReference({...})` 手动创建并在场景中 `App.Scene.Add()`，从而改变全局坐标系的原点或建立局部坐标原点。这在导入外部 CAD 数据时非常重要。
...
**使用流程**：
1. **启动取线**：`App.Tools.PickerPolyline.StartPickPolyline()`
2. **监听事件**：注册 `OnPickPolylineEvent`
3. **结束取线**：`App.Tools.PickerPolyline.EndPickPolyline()`
4. **获取折线结果**：`App.Tools.PickerPolyline.GetPickedPolylines(coordZRef)` （参数 coordZRef 常为 0 代表 surface）

> 🚨 **AI 认知预警：交互工具的异步生命周期设计**
AI 在编写 `PickerPoint`、`PickerPolyline` 和 `Measure` 等**用户交互拾取工具**时，**严禁**将 `Start`、`End`、`Get` 紧凑地写在同一个同步函数中。必须明确分离生命周期：
1. 业务逻辑提供“开启”入口（调用 `Start` 并交出控制权，让用户去点选画面）；
2. 业务逻辑提供“完成”入口，或基于事件（`PickPointEvent` 等）计数判断满足条件后，再去调用 `End` 并通过 `GetPickedPoints` 取回数据绘制图层。
