...
**代码生成前必须查阅**：
- `../official_api_code_example/official-entity-general-behavior.md` - 完整 API 列表和代码示例
- `../official_api_code_example/official-bim-full.md` - BIM 相关操作（包含房间高亮等）
...
> ⚠️ **具体代码示例必须查阅**：`../official_api_code_example/official-entity-general-behavior.md`

### 批量显隐
- 通过对象批量显隐：`SetVisibleByObjects`
- 参数：`true`=显示, `false`=隐藏

### 批量删除
- 方式1：通过类型删除（不支持 ProjectModel/ProjectInstance）- `ClearByTypes`
- 方式2：通过对象删除 - `ClearByObjects`
- 方式3：通过 Eids 删除 - `ClearByEids`

### 批量更新与聚焦
- 通过 EntityName 更新 - `UpdateByEntityName`
- 通过 CustomId 更新 - `UpdateByCustomId`
- 通过 EntityName 聚焦 - `FocusByEntityName`
- 通过 CustomId 聚焦 - `FocusByCustomId`
...
// 控制移动
moveObj.SetTime(30);
moveObj.SetLoop(false);
moveObj.SetReverse(true);
moveObj.SetState('pause'); // play/pause/stop
moveObj.SetOffset({ left: 10, forward: 5, up: 0 });
moveObj.SetRotator({ pitch: 0, yaw: 30, roll: 0 });
```
