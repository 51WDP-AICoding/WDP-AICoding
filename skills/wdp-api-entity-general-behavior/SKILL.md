...
await App.Scene.SetVisibleByObjects(objs, false); // true=显示, false=隐藏
```

### 批量删除
```javascript
// 方式1：通过类型删除（不支持 ProjectModel/ProjectInstance）
await App.Scene.ClearByTypes(['Particle', 'Range']);

// 方式2：通过对象删除
const objs = [particleObj, pathObj];
await App.Scene.ClearByObjects(objs);

// 方式3：通过 Eids 删除
await App.Scene.ClearByEids(['eid1', 'eid2']);
```

### 批量更新与聚焦
```javascript
// 通过 EntityName 更新
await App.Scene.UpdateByEntityName(['name1', 'name2'], jsondata);

// 通过 CustomId 更新
await App.Scene.UpdateByCustomId(['id1', 'id2'], jsondata);

// 通过 EntityName 聚焦
await App.CameraControl.FocusByEntityName(['name1', 'name2'], {
...
// 通过 CustomId 聚焦
await App.CameraControl.FocusByCustomId(['id1', 'id2'], {
  rotation: { pitch: -30, yaw: 0 },
  distanceFactor: 0.4,
  flyTime: 1
});
