...
| 插件 | 正确版本 | 错误版本 | 后果 |
|------|---------|---------|------|
| BIM | `@wdp-api/bim-api@^2.1.1` | `^2.2.1` | No matching version found |
| GIS | `@wdp-api/gis-api@^2.1.0` | `^2.2.1` | No matching version found |

**原因**：BIM/GIS 插件的版本号与 wdpapi 核心包不同步，不存在 2.2.x 版本。不要从 `wdpapi: "^2.2.1"` 推断插件版本。
...
2.5 ✅ 安装所需插件（按需，在 Renderer.Start 之前执行）
   - 🚨 **使用 BIM 相关 API（`App.BimModel` / `App.Component` / `App.Asset` / `App.Space`）时必须安装 BIM 插件**：
     ...
     ```
     > ⚠️ BIM 插件最新版为 2.1.1，不存在 2.2.x 版本，填写 `^2.2.1` 会导致安装失败。
     ```javascript
   ...
   - 🚨 **使用 GIS 相关 API（`App.gis` / `App.GeoLayer`）时必须安装 GIS 插件**：
     ...
     ```
     > ⚠️ GIS 插件最新版为 2.1.0，不存在 2.2.x 版本，填写 `^2.2.1` 会导致安装失败。
     ```javascript
...
🚨 **使用 `import` 从 `node_modules` 导入 npm 包时，必须使用支持模块解析的构建工具/开发服务器，纯静态文件服务器无法工作。**
...
- ✅ **推荐：Vite**
  ...
  npx vite . --port 8090
  ```
- ✅ 其他可用：Webpack Dev Server、Parcel、Vite 等
- ❌ **禁止使用纯静态服务器**：`http-server`、`python -m http.server`、`live-server`、VS Code Live Server 扩展等——这些服务器不会解析 `node_modules` 中的包路径，浏览器会报 `Failed to resolve module specifier "wdpapi"` 或类似错误

> ⚠️ **常见误区**：`package.json` 中设置了 `"type": "module"` 仅表示 `.js` 文件按 ES Module 解析，但不等于浏览器能直接加载 `node_modules` 中的包。模块路径解析（bare specifier → 实际文件路径）必须由构建工具完成。
...
1. ⚠️ `Failed to resolve module specifier "wdpapi"` / `Uncaught SyntaxError: Cannot use import statement outside a module`
   - 💡 **最常见原因：使用了纯静态服务器（http-server、python -m http.server、VS Code Live Server 等）**
   - 💡 必须切换到 Vite 等支持模块解析的开发服务器：`npm install -D vite && npx vite .`
   - 💡 详见上方「2.1.1 开发服务器要求（强制）」
...
3. ⚠️ `BimApi is not defined` / `BimApi SDK 未加载`
   - 💡 确认已安装 `@wdp-api/bim-api@^2.1.1`：`npm install @wdp-api/bim-api@^2.1.1`
