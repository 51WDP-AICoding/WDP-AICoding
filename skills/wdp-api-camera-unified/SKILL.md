...
### 1. 基础相机控制
- 相机聚焦：Focus、FocusOnLocation
- 相机移动：MoveTo、RotateTo
- 获取相机信息：GetTransform、GetFrustum

### 2. 视角切换
- 预设视角切换：SwitchToPresetView
- 自定义视角保存与加载：SaveCurrentView、LoadSavedView、GetAllSavedViews

### 3. 漫游路径
- 路径加载与管理：LoadPath、GetAllPaths、GetPathInfo
- 路径漫游控制：StartPath、PausePath、ResumePath、StopPath、SetSpeed、JumpToPosition
- 路径事件监听：RegisterEvent (OnRoamPathStart, OnRoamPathEnd, OnRoamPathPause, OnRoamPathResume, OnRoamPathPositionUpdate)

### 4. 自由漫游模式
- 自由漫游控制：StartFreeRoam、StopFreeRoam、SetFreeRoamSpeed
- 自由漫游键盘控制：MoveForward、MoveBackward、MoveLeft、MoveRight、MoveUp、MoveDown、RotateUp、RotateDown、RotateLeft、RotateRight

### 5. 相机动画
- 相机动画控制：CreateAnimation、PlayAnimation、PauseAnimation、StopAnimation
