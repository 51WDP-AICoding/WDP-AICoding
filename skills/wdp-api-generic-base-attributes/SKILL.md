...
**格式规范**：
在展示 API 属性赋值时，需从官方提取展示双写格式，确保代码示例同时包含两种访问方式：
- **方式一（属性访问）**：`obj.prop = value;` —— 简单直观，适合简单读取和快速原型。
- **方式二（异步方法）**：`await obj.SetXxx(value);` —— 返回标准响应结构，适合严谨业务逻辑。

同样的，所有 getter 示例也必须呈现双写法：
- **方式一（属性访问）**：`obj.prop` —— 直接读取属性值。
- **方式二（异步方法）**：`await obj.GetXxx();` —— 返回 `{success, message, result}` 结构。

> 🚨 **注意**：方式一为同步属性访问，仅适用于简单场景；方式二为异步方法调用，返回完整的 API 响应结构，推荐用于生产环境。
