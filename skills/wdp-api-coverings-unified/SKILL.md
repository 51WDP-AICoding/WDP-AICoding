...
**代码生成前置要求**：
> 🚨 **必须阅读**：根据业务场景选择对应的 official 文档：
> - 空间标注与交互：`../official_api_code_example/official-entity-coverings-spatial.md`
> - 路径与运动：`../official_api_code_example/official-entity-coverings-path.md`
> - 数据可视化与特效：`../official_api_code_example/official-entity-coverings-effects.md`
...
> 📖 **完整覆盖物创建 API 签名**：参考对应的 official 文档（见上方分类）
...
| 场景 | 典型用法 | 关键方法 | 参考文档 |
|------|---------|---------|---------|
| **POI + Window 交互** | POI 作为可点击标记，点击后弹出 Window | `poiObj.onClick(callback)`, `new App.Window({...})`, `App.Scene.Creates([...])` | official-entity-coverings-spatial.md `[demo]POI添加Window` |
| **区域标注（Range）** | 创建多边形/圆形区域轮廓，支持 SHP/GeoJSON 导入 | `new App.Range({...})`, `obj.GetRangeInfo()`, `App.DataModel.Geometry.CreateGeometryFromShapefile/GeoJson` | official-entity-coverings-spatial.md `Topic: 区域轮廓` |
| **热力图展示** | 区域/柱状/点云/路径/3D网格热力图 | `new App.HeatMap/ColumnarHeatMap/SpaceHeatMap/RoadHeatMap/MeshHeatMap({...})`, `obj.GetHeatValue()`, `obj.Filter/UnFilter` | official-entity-coverings-effects.md `Topic: 热力图系列` |
| **路径展示（Path）** | 轨迹、路线、管网等线状数据可视化 | `new App.Path({polyline, pathStyle})`, `obj.Update()`, `obj.onClick(callback)` | official-entity-coverings-path.md `Topic: 路径` |
...
| POI 点击后未弹出 Window | 事件未绑定或参数错误 | 确认已使用 `poiObj.onClick()` 绑定事件；确认 Window 的 URL 和位置参数正确；参考 `official-entity-coverings-spatial.md` |
