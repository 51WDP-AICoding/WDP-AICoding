...
#### 3.3 批量构件专题高亮（BIM 2.2.0 新增）

```javascript
// 批量设置多个构件的高亮状态，适用于专题展示（如按楼层、按系统着色）
async function batchHighlightComponents(entity, highlightConfigs) {
  try {
    // highlightConfigs 格式：
    // [
    //   { 
    //     nodeId: "597",              // string | Array<string> 单个或多个构件ID
    //     color: "76fffc",            // 高亮颜色（HEX值）
    //     hightlight: true,           // 是否高亮
    //     opacity: 1,                 // 透明度 0.0~1.0
    //     bCanBeOccluded: true        // 是否可被遮挡
    //   }
    // ]
    await entity.SetNodesHighlight(highlightConfigs);
    
    console.log(`批量高亮设置成功，共 ${highlightConfigs.length} 个构件`);
    return true;
  } catch (error) {
    console.error(`批量设置构件高亮时出错: ${error.message || error}`);
    return false;
  }
}

// 使用示例1：按楼层高亮不同颜色的构件
async function highlightByFloor(entity, floorComponents) {
  const configs = floorComponents.map((item, index) => ({
    nodeId: item.nodeId,
    color: ["76fffc", "00B3E6", "ff6b6b", "feca57"][index % 4],
    hightlight: true,
    opacity: 0.8,
    bCanBeOccluded: true
  }));
  
  return await batchHighlightComponents(entity, configs);
}

// 使用示例2：批量高亮同一系统的多个构件
async function highlightSystem(entity, nodeIds, color) {
  // nodeIds 可以是数组，一次性高亮多个构件
  return await batchHighlightComponents(entity, [{
    nodeId: nodeIds,  // 如：["597", "598", "599"]
    color: color,
    hightlight: true,
    opacity: 1,
    bCanBeOccluded: true
...
// 使用示例3：清除所有高亮
async function clearAllHighlight(entity) {
  // 设置 hightlight: false 取消高亮
  return await batchHighlightComponents(entity, [{
    nodeId: "all",  // 特殊值："all" 表示所有构件
    hightlight: false
  }]);
}
```

**`SetNodesHighlight` vs `SetHighlight` 区别**：

| 特性 | `SetHighlight` | `SetNodesHighlight` (2.2.0+) |
|------|---------------|------------------------------|
| 适用场景 | 单个构件交互反馈 | 批量构件专题展示 |
| 性能 | 逐个调用，适合少量 | 批量处理，适合大量 |
| 功能 | 基础高亮 | 支持批量、系统级着色 |
| 推荐使用 | 点击高亮单个构件 | 楼层/系统着色、热力图效果 |

**BIM 2.2.0 更新说明**：
- 新增 `SetNodesHighlight` 批量构件专题高亮方法
- 修复构件高亮时对原有材质纹理的显示问题
- 优化构件激活性能，降低激活时间
...
- 🔗 `../official_api_code_example/official-bim-full.md` - **BIM API 完整文档（已验证）**
- 🔗 `../official_api_code_example/official-bim-core-operations.md`
- 🔗 `../official_api_code_example/official-scene-camera.md` — 相机聚焦方法真值来源（CameraControl.Focus 等）
- 🔗 `../official_api_code_example/ONLINE_COVERAGE_AUDIT.md` - 在线文档访问说明（含 API 端点和 Topic ID 对照表）
