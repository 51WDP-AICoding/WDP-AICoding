...
> 📖 **详细安装流程**：参考 `../wdp-api-initialization-unified/SKILL.md`

```javascript
import BimApi from '@wdp-api/bim-api';
await App.Plugin.Install(BimApi);  // 必须在 Renderer.Start() 之前
```
...
> 💡 **使用要点**：`visibility: true` = 隐藏其他节点（孤立），`visibility: false` = 显示其他节点（恢复）

#### ⚠️ 与 `SetNodeShowAll` 的编排冲突
- `SetNodeShowAll()` 是全量恢复操作，会覆盖孤立效果
- 编排建议：孤立开启/恢复优先使用 `SetOtherNodesVisibility`，全量复位时再调用 `SetNodeShowAll()`
