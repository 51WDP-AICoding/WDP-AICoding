...
## [2026-03-12] 新增辅助规则：链式调用出入参不确定时强制暂停

### 背景
在实际开发中，AI 可能在出入参不明确的情况下猜测参数结构并直接提交实现，导致链式调用失效。
新增强制规则，要求在识别到不确定的方法时主动暂停，向用户提问并补充文档后再继续。

### 更新文件

#### `skills/wdp-entry-agent/SKILL.md`
- 在"当前盲区输入清单"中新增**第6条：链式调用中存在不确定的方法出入参（强制暂停）**
- 规则内容：识别出需要链式调用时，若任一方法的入参结构、出参字段、枚举值或调用时序不明确，必须主动暂停开发
- 暂停时向用户说明哪个方法不确定，并提示"可联系产研人员补充描述"
- 获取答案后，将补充内容更新到对应 `official-*.md` 的注释或调用规则说明中，再继续实现
- 禁止在出入参不确定的情况下猜测参数结构并提交为最终实现

#### `skills/README.md`
- 在"关键参数确认规则"中补充链式调用暂停规则的简要说明
