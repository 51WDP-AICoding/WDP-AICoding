...
### 解绑单个场景事件
- **方法**：`App.Renderer.UnRegisterSceneEvent(['EventName'])`
- **说明**：解绑指定的场景事件

> 📖 **完整 API 签名**：参考 `../official_api_code_example/official-general-event-registration.md`

### 解绑云渲染事件
- **方法**：`App.Renderer.UnRegisterEvent(['eventName1', 'eventName2'])`
- **说明**：解绑云渲染相关事件
...
### 重复注册事件机制
- **注册**：`RegisterSceneEvents`（支持同一事件多次注册）
- **获取**：`GetRegisterSceneEvents`
- **解绑**：`UnRegisterSceneEvents`
